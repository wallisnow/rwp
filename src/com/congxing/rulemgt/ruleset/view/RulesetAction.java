...
public class RulesetAction extends BaseAction {
	...
	public String list(){
		try{
			...
			StringBuffer sql = new StringBuffer("select ruleset_id rulesetId,ruleset_name rulesetName,ruleset_type rulesetType, ruleset_desc rulesetDesc  from RM_RULESET where 1=1 ");
			String[] param = null;
			this.setParamsMapToTargetObject(paramsMap, listVO);
			if(StringUtils.isNotBlank(listVO.get_seq_rulesetType())&&StringUtils.isNotBlank(listVO.get_sk_rulesetName())){
			...
			List<RulesetVO> datas = this.getService().doQueryList(sql.toString(), RulesetVO.class, listVO.getPageNo()-1,listVO.getPageSize() ,  param);
	...
	public String save(){
		try{
			...
			if(Constants.STRING_TRUE.equals(isNew)){
				...
				entityVO = this.getService().doCreate(voClass, vo);
				this.addInfoMessage("保存成功");
			}else{
				List<RulesetVO> oldList = (List<RulesetVO>)this.getService().doFindByProperty(voClass, "rulesetId", vo.getRulesetId());
				for(RulesetVO oldVO : oldList){
					...
					entityVO = this.getService().doUpdate(voClass, oldVO);
				}
		...
		try {
			...
			if(null != pkValues && pkValues.length > 0){
				for (int index = 0; index < pkValues.length; index++) {
					RulesetVO rulesetVO = (RulesetVO)this.getService().doFindByPK(RulesetVO.class, PkUtils.getPkVO(RulesetVO.class, pkNameArray, pkValues[index]));
					rulesetVO.setStatus(Constants.RULESET_STATUS_USING);
					this.getService().doUpdate(RulesetVO.class, rulesetVO);
				}
			}
			this.sendJsonMessage(new JsonResult(ActionSupport.SUCCESS, "操作成功"));
		} catch (Exception ex) {
			ex.printStackTrace();
			this.sendJsonMessage(new JsonResult(ActionSupport.ERROR, ex
					.getMessage()));
		}
	}
	...
	public String editRulesetCfg(){
		try{
			...
			RulesetVO rulesetVO = (RulesetVO) this.getService().doFindByPK(RulesetVO.class, PkUtils.getPkVO(voClass, pkNameArray, rulesetPK));
			if(null == rulesetVO){
	...
	public String listRule(){
		try{
			...
			RulesetVO rulesetVO = (RulesetVO) this.getService().doFindByPK(voClass, PkUtils.getPkVO(voClass, pkNameArray, rulesetPK));
			if(null == rulesetVO){
				throw new Exception("规则集信息不存在");
			}
	...
	public String addRule(){
		try{
			...
			//初始化数据
			ruleVO.setRuleZhName("规则_"+DateFormatFactory.getInstance("yyyyMMdd_HHmm").format(new Date()).replace("-", ""));
			ruleVO.setRuleEnName("RULE_"+DateFormatFactory.getInstance("yyyyMMdd_HHmm").format(new Date()).replace("-", ""));
	...
	public String editRule(){
		try{
			...
			RuleVO ruleVO = (RuleVO) this.getService().doFindByPK(RuleVO.class, PkUtils.getPkVO(RuleVO.class, "ruleId", ruleId));
			if(null == ruleVO){
				throw new Exception("规则集信息不存在");
			}
	...
	public void listRulesetFunJson() throws IOException {
		try{
			...
			List<RulesetFunVO> boDates = (List<RulesetFunVO>) this.getService().dofindDatas(RulesetFunVO.class, funListVO);
	...
	public void testRulesetJson() throws IOException{
		try{
			...
			List<RulesetBoVO> boDatas = (List<RulesetBoVO>) this.getService().doFindByHQL(hql, new Long(pkArr[0]), pkArr[1]);
