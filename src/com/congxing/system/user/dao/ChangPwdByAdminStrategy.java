...
public class ChangPwdByAdminStrategy implements Strategy {
	...
	public ChangPwdByAdminStrategy(String userId, String newPwd, UserVO adminUserVO) {
		this.userId = userId;
		this.newPwd = newPwd;
	...
	public Object process() throws Exception {
		...
		UserVO thisVO = (UserVO)userDAO.findByPK(userId);
		...
		if(null != hisPwdVO){
			...
			thisVO.setPassword(MD5Encrypt.getMD5Password(newPwd));
			thisVO.setInitPwd(Constants.TYPE_NO);//非初始密码
			thisVO.setModifyTime(new Date());
			...
			logVO.setOprCode(adminUserVO.getUserId());
			logVO.setOprType(Constants.OPRTYPE_MODIFY);
			logVO.setOprTime(new Date());
			BeanUtils.copyProperties(logVO, thisVO);			
			
			userLogDAO.save(logVO);
			userDAO.saveOrUpdate(thisVO);
