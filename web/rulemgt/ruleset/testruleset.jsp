...
<head>
	...
	$(function(){
		...
		for(var idx = 0; idx < boDatas.length; idx++){
			...
			tboHtml.push("<td><input type='text' name='" + boDatas[idx].boName + "_rs' class='textInput'/></td>");
			tboHtml.push("</tr>");
		}
	...
	function doTestRulesetCfg(){
		...
		$.getJSON(ctx + "/rulemgt/ruleset/testRulesetJson.action", formObj, function(result){
			if("success" == result.status){
				var json_result = eval('(' + result.message + ')');
				for(var key in json_result){
					$("input[name=" + key + "_rs]").val(json_result[key]);
				}
			}else{
...
<div class="searchPageContent">
	...
	<div class="formBar">
		<a href="javascript:void(0);" onclick="doTestRulesetCfg();return false;" class="linkbutton"><s:text name="test"/></a>
